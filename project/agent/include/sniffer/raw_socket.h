...
typedef struct packet//packet struct
{
	u_int32_t source_ip;
	u_int32_t dest_ip;
	u_int16_t port_source;
	u_int16_t port_dest;
	u_int8_t service_type;
	u_int16_t total_len;
} *p_packet, packet1;

typedef struct ethernet_header//ethernet struct
{
...
typedef struct ip_header//ip struct
{
...
typedef struct tcp_header//tcp struct
{
	u_int16_t source;
	u_int16_t dest;
	u_int32_t seq;
...
typedef struct udp_header//udp struct
{
	u_int16_t source;
	u_int16_t dest;
	u_int16_t len;
...
int prepare_socket();//Preparing to open the socket
unsigned short csum(unsigned short *ptr,int nbytes);
char* recv_data(int sock);//recv data
pether_h free_ether(char* buf);//save in variable and print the ether header
